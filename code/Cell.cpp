...
void Cell:: SaveCellInfo(ofstream & OutFile ,Type type)
{
...
void Cell::ApplyAction(Grid* pGrid, Player* pPlayer)
{
	if(pGameObject)
		pGameObject->Apply( pGrid,  pPlayer);
}

bool Cell ::IsOverlapping(GameObject * newobj)
{
	if(pGameObject)
		return pGameObject->IsOverLapping(newobj);
	return false;
}