...
public:
	...
	GameObject * GetGameObject(const CellPosition & position); //Gets a pointer to the object in position 
	//===========Special attacks===============
	...
	void SaveAll(ofstream &OutFile,Type type); //to save this grid in txt file
	void CleanGrid(); // used to clean the grid when we open new one
