...
class Player
{
	...
	//	//===============Special attacks flags =======================
	bool freeze;  //to indicate if the player can move 
	int Fire_affected ;
	int  Poison_affected;
	bool Lightning;
	bool UsedAttacks[4];
	int UsedAttacksNumber;
...
public:
	...
	int GetJustRolledDiceNum();// A getter for the JustRolled Dice Num
	bool IsFreezed(); // getter for freeze
	void SetFreeze(bool ); // setter for freeze
	// ====== Drawing Functions ======
	...
	void  MoveInSameTurn(Grid *pGrid,int num);//Moves the player with passed num without changing turn count
